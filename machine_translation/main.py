...
_, source_vocab_size = load_vocab('vocab.en')
_, target_vocab_size = load_vocab('vocab.vi')
...
source_dataset = tf.data.TextLineDataset(tf.gfile.Glob('tst2012.en'))
target_dataset = tf.data.TextLineDataset(tf.gfile.Glob('tst2012.vi'))
source_vocab = lookup_ops.index_table_from_file(
  'vocab.en', default_value=UNK_token)
target_vocab = lookup_ops.index_table_from_file(
  'vocab.vi', default_value=UNK_token)
...
(source_sequence,
 ...
 source_sequence_length, target_sequence_length) = iterator.get_next()
